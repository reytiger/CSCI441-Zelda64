 ...
 *
 *      Of particular use, it uses the Object class to load in a simple
 *WaveFront
...
GLint leftMouseButton  = GLUT_UP,
      rightMouseButton = GLUT_UP; // status of the mouse buttons
int mouseX = 0, mouseY = 0; // last known X and Y of the mouse

float cameraTheta, cameraPhi,
    cameraRadius; // camera position in spherical coordinates
...
//  on its position in spherical coordinates. The user passes in the current
//  phi,
//  theta, and radius, as well as variables to hold the resulting X, Y, and Z
//  values.
//  Those variables for X, Y, and Z values get filled with the camera's position
//  in R3.
//
void computeArcballPosition(float theta, float phi, float radius, float &x,
                            float &y, float &z) {
...
void initScene() {
    ...
    // by default, OpenGL has a little bit of ambient light throughout the scene
    // {0.2}
...
void renderScene(void) {
    ...
    glMatrixMode(
        GL_MODELVIEW); // make sure we aren't changing the projection matrix!
    glLoadIdentity();

    gluLookAt(cameraX + objX,
              cameraY + objY,
              cameraZ + objZ, // camera is located at (x,y,z)
              objX,
              objY,
              objZ, // camera is looking at (0,0,0)
              0.0f,
              1.0f,
              0.0f); // up vector is (0,1,0) (positive Y)
    ...
    GLfloat ambCol[4]
        = {0.8, 0.8, 0.8}; // this value can be large because the final value
    ...
    glMaterialf(
        GL_FRONT_AND_BACK,
        GL_SHININESS,
        96.0); // as well as the shininess - this value ranges between 0 & 90
...
void normalKeys(unsigned char key, int x, int y) {
    ...
    case 'L':
        ...
        if (lightType == POINT_LIGHT) { // if a point light
            ...
            // we mostly need to set the pos[3] "w" value to 0.0 to signify it
            // is a vector
            ...
            // return the angle of our cone back to 180 to signify back to point
            // light
...
void mouseMotion(int x, int y) {
    ...
    if (leftMouseButton == GLUT_DOWN) {
        ...
        // phi, though.. it'll flip out if it flips over top. keep it in (0,
        // M_PI)
        if (cameraPhi <= 0)
            cameraPhi = 0 + 0.001;
        if (cameraPhi >= M_PI)
        ...
        computeArcballPosition(
            cameraTheta, cameraPhi, cameraRadius, cameraX, cameraY, cameraZ);
    } else if (rightMouseButton == GLUT_DOWN) {
        // for the right mouse button, just determine how much the mouse has
        // moved total.
        // not the best "zoom" behavior -- if X change is positive and Y change
        // is negative,
        //(along the X = Y line), nothing will happen!! but it's ok if you zoom
        //by
        // moving left<-->right or up<-->down, which works for most people i
        // think.
        double totalChangeSq = (x - mouseX) + (y - mouseY);
        cameraRadius += totalChangeSq * 0.01;

        // limit the camera radius to some reasonable values so the user can't
        // get lost
        if (cameraRadius < 2.0)
            cameraRadius = 2.0;
...
int main(int argc, char **argv) {
    ...
    // create a double-buffered GLUT window at (50,50) with predefined
    // windowsize
    glutInit(&argc, argv);
